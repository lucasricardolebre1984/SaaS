...
## 4.2.1) Premium Layout Matrix
- `layout1`
  - papel: institutional premium padrao;
  - sensacao: produto enterprise de operacao diaria, denso, confiavel, vendavel;
  - direcao: dark polished, contraste alto, cards bem recortados, hierarquia clara, menos "efeito showcase" e mais cockpit executivo.
- `layout2`
  - papel: flagship visual;
  - sensacao: neural/dark/high-end;
  - direcao: glass control room, brilho controlado, profundidade, motion e glow discretos, foco em wow factor sem perder uso diario.
- `layout3`
  - papel: executive light enterprise;
  - sensacao: CRM premium claro, limpo, serio e comercial;
  - direcao: superfícies claras, sombras finas, bordas elegantes, leitura imediata e aparencia de software caro.

Regra:
- layout muda estrutura, ritmo, superficies e linguagem de componente;
- paleta muda cor/acento;
- layout nunca deve ser implementado como simples troca de background.
...
## 4.4) Front Premium Hardening
- Fonte unica de verdade do visual:
  - modulo 06 controla `layout` e `palette`;
  - surfaces embutidas devem obedecer ao estado passado pelo owner;
  - runtime embutido nao pode competir com `localStorage` legado.
- Componentes premium obrigatorios:
  - `command-bar`
  - `board-tabs`
  - `kpi-card`
  - `analytics-panel`
  ...
  - `detail-drawer`
  - `config-surface`
- Gates visuais minimos:
  - paridade Owner/CRM por layout;
  - estados hover/focus/active claros;
  - sem botoes "mortos";
  - sem regressao desktop/mobile.
